...
func basictest(s store.Store, t *testing.T) {
	...
	if err := s.Delete(ctx, "foo", func(d *store.DeleteOptions) {}); err != nil {
	...
	s.Write(ctx, &store.Record{Key: "a", Value: []byte("a")})
	s.Write(ctx, &store.Record{Key: "aa", Value: []byte("aa")})
	s.Write(ctx, &store.Record{Key: "aaa", Value: []byte("aaa")})
	if results, err := s.Read(ctx, "b", store.ReadPrefix()); err != nil {
		t.Error(err)
	} else {
		if len(results) != 0 {
